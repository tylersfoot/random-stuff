...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var color = vec4<f32>(
        f32((params.color_fill >> 16u) & 0xFFu) / 255.0,
        f32((params.color_fill >> 8u) & 0xFFu) / 255.0,
        f32(params.color_fill & 0xFFu) / 255.0,
        1.0
    );
    if (dot(z, z) > threshold_squared) {
        // outside the set, color will be interpolated
        ...
        color = vec4<f32>(r / 255.0, g / 255.0, b / 255.0, 1.0);
    }

    // textureStore(texture, coordinates, color_value)
