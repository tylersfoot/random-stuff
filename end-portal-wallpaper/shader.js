...
vec4 end_portal_layer(float layer, vec4 proj) {
  ...
  proj.xy *= 0.5; // zooms in, basically
...
document.addEventListener('DOMContentLoaded', () => {
  var canvas = document.querySelector('#end-portal-canvas');
...
// function render() {
//   var shaderCurrentTime = performance.now();
...
//     shaderParallaxCurrent[0] = lerp2(shaderParallaxCurrent[0], shaderParallaxTarget[0], shaderParallaxAmount);
//     shaderParallaxCurrent[1] = lerp2(shaderParallaxCurrent[1], shaderParallaxTarget[1], shaderParallaxAmount);
...
//   gl.uniform1f(shaderAnimationTimeUniformLocation, ((shaderCurrentTime * idleSpeed) % 1200000) / 1200000);

//   gl.drawArrays(gl.TRIANGLES, 0, 6);

//   while (shaderCurrentTime - shaderLastTime < 1000 / fps) {
//     shaderCurrentTime = performance.now();
//   }

//   shaderLastTime = shaderCurrentTime;
//   // request the next frame
//   requestAnimationFrame(render);
// }

function render(currentTime) {
  var shaderTimeSinceLastFrame = currentTime - shaderLastTime;

  if (shaderTimeSinceLastFrame > (1000 / fps)) {
    shaderLastTime = currentTime - (shaderTimeSinceLastFrame % (1000 / fps));
    ...
    gl.uniform1f(shaderAnimationTimeUniformLocation, ((currentTime * idleSpeed) % 1200000) / 1200000);
